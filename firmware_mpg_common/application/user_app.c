...
static void UserAppSM_ChannelOpen(void)          
{
  ...
  if( AntReadData() )
  {
    ...
    else if(G_eAntApiCurrentMessageClass == ANT_TICK)
    {
      ...
      /*check the tick message contents: look at the EVENT CODE*/
      if(u8LastState != G_au8AntApiCurrentData[ANT_TICK_MSG_EVENT_CODE_INDEX])
      {
        /*the state has changed,so update u8LastState and queue a debug message*/
...
/*-------------------------------------------------------------------------------------------------------------------*/
/* wait for ant to close a channel */
